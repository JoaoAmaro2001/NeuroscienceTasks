...
% Number of trials
n           = data.task.stims_per_run; 
% Init task control vars
begin_task  = false;                      % Flag to begin task
tr_final    = n*(1+21+2+2+1)/data.mri.tr; % Number of triggers == 495
...
event_table  = table('Size', [numEvents, 4],...
...
if data.output.export_xlsx
    ...
    writetable(logTable, [data.dir.logs_path filesep output.log_file_name '.xlsx']);
end

if data.output.export_tsv
    % Write the log table to a TSV file
    writetable(logTable, [data.dir.logs_path filesep output.log_file_name '.tsv'], 'FileType', 'text', 'Delimiter', '\t');
end
